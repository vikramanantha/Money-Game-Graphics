...
def rulez():
    ...
    daRulez = Label(text='The objective of The Republican Way is to earn all the money you can.\nPressing the \'$\' gets'
...
def showDemStuff():
    global showBuy
    global showHire
    showStuff.config(text="Don't Show Things", command=lambda: getridofbox(showBuy, showHire))
    showBuy = Listbox(win)
    showBuy.pack(anchor=S)
    showBuy.insert(END, "You have bought ",)
    for i in boughtItems:
        showBuy.insert(END, i)
    showHire = Listbox(win)
    showHire.pack(anchor=S)
    showHire.insert(END, "You have hired ")
    for i in HiredItems:
        showHire.insert(END, i)

def getridofbox(showBuy, showHire):
    showBuy.config(state=DISABLED)
    showHire.config(state=DISABLED)
    showBuy.pack_forget()
    showHire.pack_forget()
    showStuff.config(text='Show Things', command=lambda: showDemStuff())


win = Tk()
win.geometry("1800x1200")
win.title("The Republican Way")
Title = Label(win, text="The Republican Way\n", font=("Times New Roman", 44))
Title.pack()
showMoney = Label(win, text="You have $" + str(score), font=("Times New Roman", 40))
showMoney.pack()
sala = Label(win, text='Your Salary is $' + str(rate), font=('Arial', 32))
sala.pack()
likeness = Label(text="People like you at level " + str(likenessLevel), font=('Arial', 30))
likeness.pack()
status = Label(text="Status: Not Much", font=('Arial', 20))
status.pack()
but = Button(win, text="$", font=("Agency", 24), command=EarnMoney, width=15, height=5)
but.pack()
...
HireButton.pack(anchor=NE)

ObjToHire = OptionMenu(win, HselectedObj, *hireObjNames, command=HireStuff)
ObjToHire.pack(anchor=NE)


showStuff = Button(text="Don't Show Things", command=lambda: getridofbox(showBuy, showHire))
showStuff.pack(anchor=S)
